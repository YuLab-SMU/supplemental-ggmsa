...
```{r ancestral_sequences, fig.height = 10, fig.width = 12, warning = FALSE,message=FALSE, fig.cap="\\label{fig:ancestral_sequences}**Displaying ancestral sequences in tree-MSA plot**. Different display modes of treeMSA_plot(): (A) Only displaying the sequence corresponding to the selected ancestral node; (B) Displaying all ancestral sequences of the subtree. (C) Displaying corresponding ancestor sequences of multiple selected internal nodes."}
...
p3 <- treeMSA_plot(p, 
                   tidymsa, 
                   ancestral_node = c(11,14,16,17), 
                   sub = FALSE, 
                   color = "Chemistry_NT",
                   seq_colname = "mol_seq")


pp <- plot_list(gglist = list(p1,p2,p3),
                ncol = 1,
                tag_levels = "A",
                heights = c(0.4,0.7,0.6))
pp
```
...
## Fig. \ref{fig:pp_all}: simplified and global arc diagram of ACE2-RBD interaction

```{r pp_all, fig.height=8, fig.width=10, warning=FALSE, message=FALSE, fig.cap="\\label{fig:pp_all}**Arc diagram of ACE2-RBD interaction**. The two sequences on the left panel are ACE2, and the two sequences on the right panel are RBD sequences of S protein of wild-type and Omicron, respectively. ACE2 and RBD are placed in the same horizontal position, and the residual sites that generated hydrogen bonds were connected by arcs. The upper arc represents the ACE2-Omicron_RBD and the lower arc represents the ACE2-WT_RBD.The main arc plot only shows the residues at the interaction sites, and the global arc diagram is shown in the upper right corner. Data of interacting sites was obtained from the PDBsum database, and the corresponding PDB ID were 7WPB and 6M0J."}
#read sequences
...
require(patchwork)
p_sim + inset_element(p_interactive, 0.7, 0.7, 1.1, 1.1) 
...
## Fig. 3A: Visual methods of surveying RNA co-variation (R code)
...
## Fig. 6B: Another example of graphics combination (R code)
